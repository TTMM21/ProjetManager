...
class Projects {
  ...
  */
  ...
  /**
  * Gives the values to nom, version, date_de_debut, date_de_fin, id_equipes
  ...
  */
  public function createProject (Project $project): bool {
    $statement = $this->pdo->prepare('INSERT INTO projets (nom, version, date_de_debut, date_de_fin, id_equipes) VALUES (?, ?, ?, ?, ?)');
  ...
  */
  public function updateProject (Project $project): bool {
    $statement = $this->pdo->prepare('UPDATE projets SET nom=?, version=?, date_de_debut=?, date_de_fin=?, id_equipes=? WHERE id_projets=?');
    return $statement->execute([
      $project->getNameProject(),
      $project->getVersionProject(),
      $project->getStartProject()->format('Y-m-d'),
